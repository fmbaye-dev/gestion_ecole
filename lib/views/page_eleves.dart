...
// ════════════════════════════════════════════════════════════════════════════
// PAGE ÉLÈVES — avec filtre par classe
// ════════════════════════════════════════════════════════════════════════════
...
class _PageElevesState extends State<PageEleves> {
  String _q = '';
  String? _filtreIdClasse; // null = toutes les classes
        ...
        children: [
                        ...
                        child: Container(
                          ...
                          child: DropdownButton<String?>(
                            ...
                            items: [
                              ...
                              ...cvm.classes.map((c) => DropdownMenuItem<String?>(
                                    value: c.id,
                                    child: Row(children: [
                                      Icon(Icons.class_rounded,
                ...
                if (_filtreIdClasse != null)
                  Padding(
                    ...
                    child: Row(children: [
                      ...
                      Text(
                        cvm.classes
                            .firstWhere((c) => c.id == _filtreIdClasse,
                                orElse: () =>
                                    ClasseModel(nomClasse: ''))
                            .nomClasse,
                        style: TextStyle(
                            fontSize: 12,
          ...
          Expanded(
            child: StreamBuilder<List<EleveModel>>(
              // Si une classe est sélectionnée → stream filtré
              ...
              builder: (_, snap) {
                ...
                final list = (snap.data ?? [])
                    .where((e) =>
                        _q.isEmpty ||
                        e.nomComplet.toLowerCase().contains(_q))
                    .toList();
        ...
        subtitle: Column(
          ...
          children: [
            ...
            Row(children: [
              ...
              if (eleve.nomClasse.isNotEmpty && eleve.anneeScolaire.isNotEmpty)
  ...
  void _confirmerSuppression(BuildContext context, EleveViewModel vm) {
    ...
    showDialog(
      ...
      builder: (ctx) => AlertDialog(
        ...
        content:
            Text('${eleve.nomComplet} sera supprimé définitivement.'),
        actions: [
          TextButton(
...
// ════════════════════════════════════════════════════════════════════════════
// FORMULAIRE AJOUT / MODIFICATION — avec anneeScolaire + erreur email
// ════════════════════════════════════════════════════════════════════════════
...
class _FormulaireEleveState extends State<FormulaireEleve> {
  ...
  void initState() {
    ...
    // Pré-remplir avec l'année courante si pas encore définie
          ...
          child: ListView(
            ...
            children: [
              _DItem(Icons.dashboard_rounded, 'Tableau de bord',
                  Routeur.routeAccueil, isActive: false),
              const _DLabel('GESTION'),
              _DItem(Icons.school_rounded, 'Élèves', Routeur.routeEleves,
                  isActive: true),
              _DItem(Icons.person_rounded, 'Enseignants',
                  Routeur.routeEnseignants, isActive: false),
              _DItem(Icons.class_rounded, 'Classes', Routeur.routeClasses,
                  isActive: false),
              _DItem(Icons.book_rounded, 'Matières', Routeur.routeMatieres,
                  isActive: false),
              const _DLabel('ACADÉMIQUE'),
              _DItem(Icons.star_rounded, 'Notes', Routeur.routeNotes,
                  isActive: false),
              _DItem(Icons.event_busy_rounded, 'Absences',
                  Routeur.routeAbsences, isActive: false),
              Divider(height: 20, color: scheme.outlineVariant.withOpacity(0.4)),
              _DItem(Icons.person_outline_rounded, 'Mon Profil',
                  Routeur.routeProfil, isActive: false),
            ],
          ),
        ),
        Padding(
