...
class _PageAccueilState extends State<PageAccueil> {
  ...
  Widget _dashboardCards() {
    ...
    final futureEleves = FirebaseFirestore.instance
        .collection('utilisateur')
        .where('role', isEqualTo: 'eleve')
        .get();
    final futureEnseignants = FirebaseFirestore.instance
        .collection('utilisateur')
        .where('role', isEqualTo: 'enseignant')
        .get();
    final futureClasses = FirebaseFirestore.instance.collection('classe').get();
    final futureUtilisateurs = FirebaseFirestore.instance
        .collection('utilisateur')
        .get();
          ...
          Expanded(
            child: ListView(
              ...
              children: [
                ...
                // ✅ BUG #1 CORRIGÉ : Bulletins maintenant visible depuis le dashboard
                _DItem(Icons.description_rounded, 'Bulletins', '/bulletin'),
                Divider(
                  height: 20,
...
// BUG #7 CORRIGÉ : utilisation d'une instance Firebase secondaire pour ne
// pas déconnecter l'admin lors de la création d'un compte
// ════════════════════════════════════════════════════════════════════════════
...
class _FormulaireAdminState extends State<_FormulaireAdmin> {
  ...
  Future<void> _creerAdmin() async {
    ...
    try {
      ...
      // pour ne pas changer l'utilisateur courant (l'admin)
      ...
      // Déconnecter et supprimer l'instance secondaire immédiatement
      ...
      switch (e.code) {
        ...
        case 'invalid-email':
          ...
          break;
        default:
          msg = e.message ?? 'Erreur de création.';
      }
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
